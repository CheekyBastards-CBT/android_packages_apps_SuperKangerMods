        ...
        public void onClick(View v) {
            switch (this.sourcetype) {
                case APPPICKER_APP:
                    ...
                    Settings.Global.putString(context.getContentResolver(), VRToxinActivity.mPrefKey, target);
                    myHandler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            ((VRToxinActivity) context).onBackPressed();
                        }
                    }, 400);
                    return;
                ...
                case APPPICKER_CUSTOM:
                    myHandler.removeCallbacksAndMessages(null);
                    String mEntry = null;
                    String[] mKeys = context.getResources().getStringArray(VRToxinActivity.mKeyArray);
                    ...
                    switch (this.position) {
                        case 0:
                            ...
                            break;
                        case 1:
                            Intent fragintent = new Intent(context, VRToxinActivity.class);
                            fragintent.putExtra(VRToxinActivity.EXTRA_START_FRAGMENT, 0);
                    ...
                    Settings.Global.putString(context.getContentResolver(), VRToxinActivity.mPrefKey, mEntry);
                    myHandler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            ((VRToxinActivity) context).onBackPressed();
